...
BEGIN
    ...
    IF VERIFICAR_AVION(ID) = FALSE THEN
        IF VERIFICAR_CAPACIDAD(CAPAC) = TRUE THEN
            ...
            INSERT INTO AVION VALUES(ID,MODELOA,CAPAC,ANIOA);
            COMMIT;
...
-- CREACION DE PROCEDIMIENTO PARA INSERTAR DATOS A LA TABLA ENCUESTA

CREATE OR REPLACE PROCEDURE INSERTAR_ENCUESTA 
(
	ID_E IN ENCUESTA.ID_ENCUESTA%TYPE,
	ID_B IN ENCUESTA.ID_BOLETA%TYPE,
	VALORACION_E IN ENCUESTA.VALORACION%TYPE,
	SUGERENCIA_E IN ENCUESTA.SUGERENCIA%TYPE

)
IS 
	BOLETA_NO_EXISTE EXCEPTION;
	ID_ENCUESTA_EXISTE EXCEPTION;
BEGIN 
	IF VERIFICAR_BOLETA(ID_B)=TRUE THEN
		IF VERIFICAR_ENCUESTA(ID_ENCUESTA)=FALSE THEN
			LOCK TABLE ENCUESTA IN ROW EXCLUSIVE MODE;
			INSERT INTO ENCUESTA
			VALUES (ID_E,ID_B,VALORACION_E,SUGERENCIA_E);
			COMMIT;

		ELSE 	
			RAISE ID_ENCUESTA_EXISTE;
	...
	EXCEPTION
		...
		WHEN ID_PAGO_EXISTE THEN
			DBMS_OUTPUT.PUT_LINE('ID DE LA ENCUESTA INGRESADA YA EXISTE');
			ROLLBACK;	
		WHEN OTHERS THEN
			DBMS_OUTPUT.PUT_LINE('HA OCURRIDO UN ERROR'|| SQLERRM);
			ROLLBACK;
...
-- CREACION DE PROCEDIMIENTO PARA BORRAR DATOS A LA TABLA ENCUESTA 

CREATE OR REPLACE PROCEDURE BORRAR_ENCUESTA
(
   	ID_E IN ENCUESTA.ID_ENCUESTA%TYPE,
	ID_B IN ENCUESTA.ID_BOLETA%TYPE,
	VALORACION_E IN ENCUESTA.VALORACION%TYPE,
...
IS 
    ENCUESTA_NO_EXISTE EXCEPTION;
BEGIN 
	IF VERIFICAR_ENCUESTA(ID_E)= TRUE THEN 
		LOCK TABLE ENCUESTA IN ROW EXCLUSIVE MODE;
		DELETE FROM ENCUESTA WHERE ID_E=ENCUESTA.ID_ENCUESTA;
		COMMIT;
	ELSE
		RAISE ENCUESTA_NO_EXISTE;
	END IF;

	EXCEPTION
        WHEN ENCUESTA_NO_EXISTE THEN 
			RAISE_APPLICATION_ERROR(-20001,'ENCUESTA QUE DESEA ELIMINAR NO EXISTE');
			ROLLBACK;
...
CREATE OR REPLACE PROCEDURE ACTUALIZAR_ENCUESTA
(
    ID_E IN ENCUESTA.ID_ENCUESTA%TYPE,
	ID_B IN ENCUESTA.ID_BOLETA%TYPE,
	VALORACION_E IN ENCUESTA.VALORACION%TYPE,
...
BEGIN 
	IF VERIFICAR_ENCUESTA(ID_E)= TRUE THEN 
		LOCK TABLE ENCUESTA IN ROW EXCLUSIVE MODE;
        UPDATE ENCUESTA
        SET ENCUESTA.ID_ENCUESTA=ID_E,
			ENCUESTA.ID_BOLETA=ID_B,
			ENCUESTA.VALORACION= VALORACION_E,
			ENCUESTA.SUGERENCIA= SUGERENCIA_E
        WHERE ID_E = ENCUESTA.ID_ENCUESTA;
		COMMIT;
	ELSE
		RAISE ENCUESTA_NO_EXISTE;
	END IF;
	...
	EXCEPTION
        WHEN ENCUESTA_NO_EXISTE THEN 
			RAISE_APPLICATION_ERROR(-20001,'ENCUESTA QUE DESEA ACTUALIZAR NO EXISTE');
			ROLLBACK;

		WHEN OTHERS THEN 
			RAISE_APPLICATION_ERROR(-20001,'ERROR SISTEMA');
			ROLLBACK;
END ACTUALIZAR_ENCUESTA;

-- TRIGGER PARA DEJAR EN MAYUSCULA LOS ATRIBUTOS DE ENCUESTA

CREATE OR REPLACE TRIGGER MAYUSCULA_ENCUESTA
BEFORE INSERT ON ENCUESTA
FOR EACH ROW
DECLARE
BEGIN
    :NEW.ID_ENCUESTA := UPPER(:NEW.ID_ENCUESTA);
    :NEW.SUGERENCIA := UPPER(:NEW.SUGERENCIA);
END;
...
BEGIN
    IF VERIFICAR_EMAIL(EMAIL_I) = TRUE THEN
        ...
        IF CONTRASENA_I = CONTRASENA_ALMACENADA THEN
                ...
                MENSAJE := 'HA INICIADO SESIÓN EXITOSAMENTE COMO ADMINISTRADOR';
...
BEGIN
    ...
    ELSE
        INSERT INTO CLIENTE VALUES ('X',NOMBRE_U, APELLIDO_U, NULL, NULL,NULL,NULL,NULL,NULL,NULL,NULL, EMAIL_U, CONTRASENA_U,NULL);
        COMMIT;
        RESULTADO := 'TRUE';
...
insert into cliente (id_cliente,email,contrasena,rol) values ('0','sc@gmail.com','sc123','ADMIN');
select * from cliente;
